...
class AddressCubit extends Cubit<AddressState> {
  ...
  Future<void> addAddress(context) async {
    ...
    var result = await addressRepoImpl.addAddressData(
  ...
  Future<void> fetchAddress() async {
    emit(AddressLoadingState());
    var result = await addressRepoImpl.fetchAddressData();
    result.fold((l) {
  ...
  Future<void> deleteAddress(context, {required String id}) async {
    emit(AddressLoadingState());
    var result = await addressRepoImpl.deleteAddressData(id);
    result.fold((l) {
  ...
  Future<void> updateAddress({required BuildContext context,required String id}) async {
    emit(AddressLoadingState());
    var result = await addressRepoImpl.updateAddressData(
      id,
      country: countryController.text,
      city: cityController.text,
      region: regionController.text,
